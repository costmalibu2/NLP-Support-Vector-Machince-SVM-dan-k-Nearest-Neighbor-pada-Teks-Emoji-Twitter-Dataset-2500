...
trained = pd.read_excel('dataset-emosi-twitter-2500.xlsx')
test = pd.read_excel('dataset-emosi-twitter-2500.xlsx')

"""# **Preprocessing**"""

trained['TEKS'] = trained['TEKS'].str.lower() # Ensuring all words in the TEKS column of training data are lowercased
test['TEKS'] = test['TEKS'].str.lower() # Ensuring all words in the TEKS column of test data are lowercased
...
# Removing all stopwords from all the TEKSs in training data.
trained["TEKS"] = trained["TEKS"].apply(lambda func: ' '.join(sw 
                                          for sw in func.split() 
                                          if sw not in stopwords))

# Removing all stopwords from all the TEKSs in test data.
test["TEKS"] = test["TEKS"].apply(lambda func: ' '.join(sw 
                                          for sw in func.split() 
                                          if sw not in stopwords))
trained.head()
...
#Training Data
trained['TEKS'] = trained['TEKS'].str.replace(r'http?://[^\s<>"]+|www\.[^\s<>"]+', '') # Removing hyperlinks from all the TEKSs. They are not needed for classification.
trained['TEKS'] = trained['TEKS'].str.replace('@[A-Za-z0-9]+', '') # Removing usernames from all the TEKSs.
trained['TEKS'] = trained['TEKS'].str.replace(r'\B#\w*[a-zA-Z]+\w*', '') # Removing hashtags, including the text, from all the TEKSs. Hashtags are useless since their words cannot be splitted with spaces.
trained['TEKS'] = trained['TEKS'].str.replace('\d+', '') # Removing numbers from all the TEKSs. They will not assist in any way to improve the classification process.

for c in special_chars:
    trained['TEKS'] = trained['TEKS'].str.replace(c,'') # Removing all special characters from all the TEKSs

#Test Data
test['TEKS'] = test['TEKS'].str.replace(r'http?://[^\s<>"]+|www\.[^\s<>"]+', '') # Removing hyperlinks from all the TEKSs
test['TEKS'] = test['TEKS'].str.replace('@[A-Za-z0-9]+', '') # Removing usernames from all the TEKSs.
test['TEKS'] = test['TEKS'].str.replace(r'\B#\w*[a-zA-Z]+\w*', '') # Removing hashtags, including the text, from all the TEKSs
test['TEKS'] = test['TEKS'].str.replace('\d+', '') # Removing numbers from all the TEKSs

for c in special_chars:
    test['TEKS'] = test['TEKS'].str.replace(c,'') # Removing all special characters from all the TEKSs
...
#Training Data
train_unique = (list(set(trained['TEKS'].str.findall("\w+").sum()))) # Finding all the unique words in training data's TEKS column
train_unique_words = len(train_unique)

#Test Data
test_unique = (list(set(test['TEKS'].str.findall("\w+").sum()))) # Finding all the unique words in test data's TEKS column
...
def cmatrix_measures(k,dists,test,cmatrix):
  ...
  for ls in dists:
    ...
    for i in knn_indices:
      label = trained['LABEL'][i] #Extracting the label of the instance by indexing it through the DataFrame.
...
def classifying(trainfeatures,testfeatures):
  ...
  y_train = trained.iloc[:,0].values #trained['LABEL]
  y_test = test.iloc[:,0].values #test['LABEL]
